  ...
  if (!email || !password || !code || !turnstileToken) {
    return response(400, {
      message: langs({
        ...
        "es-ES": "El correo, contraseña, código y token de verificación no pueden estar vacíos",
        "fr-FR": "L'email, mot de passe, code et token de vérification ne peuvent pas être vides",
        "ru-RU": "Электронная почта, пароль, код и токен подтверждения не могут быть пустыми",
        "de-DE": "E-Mail, Passwort, Code und Verifizierungstoken dürfen nicht leer sein",
    ...
    if (!user.emailVerifyCode) {
      return response(400, {
        message: langs({
          ...
          "en-US": "Verification code expired or does not exist, please request a new one",
          "es-ES": "Código de verificación expirado o no existe, solicita uno nuevo",
          "fr-FR": "Code de vérification expiré ou inexistant, veuillez en demander un nouveau",
          "ru-RU": "Код подтверждения истек или не существует, запросите новый",
          "ja-JP": "認証コードが期限切れまたは存在しません。新しいコードをリクエストしてください",
          "de-DE": "Bestätigungscode abgelaufen oder existiert nicht, bitte einen neuen anfordern",
    ...
    if (isCodeExpired(user.emailVerifyCode)) {
      return response(400, {
        message: langs({
          ...
          "fr-FR": "Le code de vérification a expiré, veuillez en demander un nouveau",
    ...
    try {
      ...
      return response(500, {
        message: langs({
          ...
          "fr-FR": "Échec de la réinitialisation du mot de passe, veuillez réessayer plus tard",
