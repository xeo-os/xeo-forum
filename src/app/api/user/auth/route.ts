  ...
  try {
    ...
    if (refreshToken) {
      ...
      try {
        ...
        if (typeof verifiedToken === "object" && verifiedToken !== null && "uid" in verifiedToken && "lastUseAt" in verifiedToken) {
      ...
      if (tokenInfo) {
        ...
        if (result) {
          ...
          return response(200, {
            ...
            // @ts-expect-error ass
            jwt: token.sign({ inner: pack(result, startTime), expired: expiredTime || "7d" }),
          });
        } else {
          return response(420, {
            message: langs(
