...
export async function POST(request: Request) {
    ...
    try {
        ...
        if (status == 'DONE') {
            ...
            console.log(task)
            const user = task?.reply?.post?.User || task?.reply?.parentReply?.user;
            ...
            if (user) {
                ...
                // 获取原始内容并截断
                const originalContent = taskDetails?.reply?.post?.[`title${langSuffixForContent}`] ||
                    taskDetails?.reply?.post?.title ||
                    taskDetails?.reply?.parentReply?.[`content${langSuffixForContent}`] ||
                    taskDetails?.reply?.parentReply?.content || '';
                
                const replyContent = taskDetails?.reply?.[`content${langSuffixForContent}`] || 
                    taskDetails?.reply?.content || '';
                ...
                await messager(
                    {
                        ...
                        link: task.reply
                            ...
                            : `https://xeoos.net/post/${task?.reply.post?.id}`,
