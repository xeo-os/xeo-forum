...
import { RiLockLine, RiMailLine, RiEyeLine, RiEyeOffLine } from '@remixicon/react';
...
export default function ResetPasswordPage() {
    ...
    const handleResetPassword = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!password || !confirmPassword || loading || !turnstileState.isVerified) {
            if (!turnstileState.isVerified) {
                toast.error(
                    lang(
                        {
                    ...
                    lang(
                        {
                            ...
                            'es-ES': '¡Restablecimiento de contraseña exitoso! Redirigiendo...',
                            'fr-FR': 'Réinitialisation du mot de passe réussie ! Redirection...',
    ...
    const renderResetStep = () => (
        <Card className='w-full max-w-md mx-auto shadow-lg'>
            ...
            <CardContent>
                ...
                <form onSubmit={handleResetPassword} className='space-y-4'>
                    ...
                    <Button
                        type='submit'
                        className='w-full'
                        disabled={loading || turnstileState.isLoading || !turnstileState.isVerified}
                    >
                        {loading ? (
                            lang(
                                {
            ...
            <CardFooter className='flex justify-center'>
                <Button variant='link' onClick={() => setStep('verify')} className='text-sm'>
                    {lang(
                        {
                            'zh-CN': '返回验证码输入',
                            'zh-TW': '返回驗證碼輸入',
